...
let x= JSON.parse(localStorage.getItem("cartvalue"))
let billing_amount=0;    
    x.map((el)=>{
        billing_amount=el.totalvalue
    })

   
let Retail_price=100000;  //scratch price
...
function cards(){
   ...
   let img4=document.createElement("img")
    ...
    let button=document.createElement("button")
        ...
        button.innerHTML=`PROCEED ${billing_amount}`
    ...
    cart_item.map((el)=>{
       ...
       let div= document.createElement("div")
        ...
        let ruppes=document.createElement("p")
            ruppes.innerHTML=`₹ ${el.prodprice}`
            ruppes.style.color="#ef3523";

        div_three.append(qty,ruppes)
...
function final_billing(){
    ...
    let tot_price_btn=document.createElement("button")
        tot_price_btn.innerHTML=`₹ ${billing_amount}`// amount needed here
